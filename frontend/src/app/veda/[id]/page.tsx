...
export default async function VedaReviewPage({ params }: { params: { id: string } }) {
      ...
      <section className="max-w-5xl mx-auto px-6 lg:px-0 py-8 lg:py-12">
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_320px] gap-16 lg:gap-24 items-start">
          <div className="order-2 lg:order-1 flex flex-col">
            ...
            {/* Amma's Marginalia */}
            <article className="border-t border-[#F5E8E0] pt-12 mb-20">
              <span className="label-classic !text-[var(--rose-accent)] block mb-8">Amma's Marginalia</span>
              <div className="space-y-6">
                {book.my_review?.split('\n').map((p: string, i: number) => (
                  <p key={i} className={`prose-classic italic opacity-80 !text-[17px] lg:!text-[19px] ${i === 0 ? 'drop-cap-rose' : ''}`}>{p}</p>
                ))}
              </div>
            </article>
            ...
            <div className="max-w-md pt-12 border-t border-[#F5E8E0] space-y-10">
              <div className="space-y-2">
                <span className="label-classic !text-[var(--rose-accent)]">Current Pairing</span>
