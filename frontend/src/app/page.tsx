...
export default async function HomePage() {
  // 1. Fetch Deep Dives from Supabase
  const { data: books } = await supabase
    .from('books')
    .select('*')
    .order('created_at', { ascending: false });

  // Logic to find the single latest for each
  const latestAmma = books?.find(b => !b.is_vedas_pick);
  const latestVeda = books?.find(b => b.is_vedas_pick);

  // 2. Fetch WordPress Articles - Updated revalidate to 0 for instant updates
  ...
  return (
    <main className="min-h-screen bg-[#FDFCFB] text-[#2C1810]">
      ...
      <div className="max-w-6xl mx-auto px-6 py-12 flex flex-col md:flex-row gap-16">
        ...
        {/* RIGHT SIDE: THE RIVER (WP Articles) */}
            ...
            {articles.map((article: any) => (
              <div key={article.id} className="group border-t border-black/5 pt-8">
                <Link href={`/articles/${article.slug}`} className="block">
                  ...
                  />
                  <div className="mt-6 text-[9px] font-bold uppercase tracking-[0.4em] opacity-40 group-hover:opacity-100 text-[#829385]">
                    Read Chapter →
                  </div>
                </Link>
              </div>
      ...
      {/* --- FOOTER --- */}
      <footer className="max-w-6xl mx-auto px-6 py-20 border-t border-black/5 grid grid-cols-1 md:grid-cols-2 gap-12 opacity-60 text-sm mt-20">
        <div>
          <h3 className="font-serif italic text-xl mb-2 text-[#2C1810]">The Library Guide</h3>
          <p className="mb-4 text-[#5a4a42]">Resources for Montreal readers and Kobo users.</p>
          <Link href="/articles/montreal-library-guide" className="font-bold uppercase text-[10px] tracking-widest border-b border-black/20 text-[#829385]">View Guide</Link>
        </div>
        <div>
          <h3 className="font-serif italic text-xl mb-2 text-[#2C1810]">The Shopping List</h3>
          <p className="mb-4 text-[#5a4a42]">Curated books for children and home collections.</p>
          <Link href="/edit" className="font-bold uppercase text-[10px] tracking-widest border-b border-black/20 text-[#829385]">Browse List</Link>
        </div>
      </footer>
    </main>
  );
